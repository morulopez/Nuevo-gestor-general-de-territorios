	...
	<div class="col-md-10 infoboxdata">
             ...
             if(!empty($campaingdata)){ ?>
             	<div class="row">
             		...
             		<div class="col-md-6 text-center">
             			<div class="divcam">
             				<i class="fas fa-bullhorn"></i> Campaña activa con el nombre <?php echo $campaingdata['nombre_campaing']; ?>
             			</div>
             		</div>
             	</div>
             <?php
			...
			<div class="row">
				<div class="col-md-6 text-left">
					<button type="button" class="btn btn-warning botonasignar" onclick="ReqDatos.req_terri_asig(<?php echo $idserviceyear["ID"];?>)"><i class="fas fa-user-plus"></i>Asignar territorio</button>
					<?php 
			...
			<div class="row">
				<div id="avi" class="col-md-6 text-left aviso">
				</div>
			</div>
			<div class="row">
					...
					<div class="row">
						<div class="col-md-8">
							<div class="divasig">
								...
								<button class="btn btn-warning buttonasignar" onclick='ReqDatos.asignarterritorio("<?php echo $idpublicador;?>")'>Asignar</button>
				...
				<!--TERRITORIOS CAMPAÑA PARA ASIGNAR-->
					...
					<div class="row">
						<div class="col-md-8">
							<div class="divasig">
								...
								<label class="labelfech" for="fechaentrega">Entrega territorio:</label>
								<input type="date" class="form-control" id="fechaentregacam">
								<label class="labelfech" for="fechadevuelta">Devuelta territorio:</label>
								<input type="date" class="form-control" id="fechadevueltacam"><br>
								<button class="btn btn-warning buttonasignar" onclick='ReqDatos.asignarterritorio_campaing("<?php echo $idpublicador;?>")'>Asignar</button>
								<button type="button" class="btn btn-warning verobservaciones" data-toggle="modal" data-target="#exampleModalCenter" onclick="ReqDatos.observaciones_asig()">Ver observaciones</button>
						    </div>
						</div>
					</div>
	...
	<div class="col-md-10 infoboxdata">
											...
											if($territorios["imagen"]==null){
											  ...
											  echo '<div class="card-body">
											    ...
											    if($territorios['devuelta']<= "20".date('y-m-d')){
											    	...
											    	$cumplidos=true;
											    	array_push($array_terri_cumplidos,["numero"=>$territorios["numero_territorio"],"zona"=>$territorios["zona"],"devuelta"=>$territorios["devuelta"],"modo"=>"servicio"]);
											    }
											    echo '</div>
											  <ul class="list-group list-group-flush">
											    <li class="list-group-item">
											    	<span class="numeroterri">Numero territorio:</span><span class="numeroterri">'.$territorios["numero_territorio"].'</span>
											    </li>
											    <li class="list-group-item">
											    	<span class="numeroterri">Zona:</span><span class="numeroterri">'.$territorios["zona"].'</span>
											    </li>
											    <li class="list-group-item">
											     	<div>
											     		<span class="numeroterri">Entrega:</span><span class="numeroterri">'.$ci->trabajar_fecha->darvuelta($territorios['entrega']).'</span>
											     	</div>
											     	<div>
											     		<span class="numeroterri">Devuelta:</span><span class="numeroterri">'.$ci->trabajar_fecha->darvuelta($territorios['devuelta']).'</span>
											     	</div>
											    </li>';?>
											    <li class="list-group-item"><button class="btn btn-secondary" type="button" onclick="ReqDatos.devolver_territorio(<?php echo $territorios['numero_territorio'];?>,<?php echo $territorios['idterritorios'];?>,<?php echo $territorios['id'];?>,'<?php echo $territorios["zona"];?>',<?php echo $territorios['id_service_year'];?>);">Devolver territorio</button></li>
											  <?php echo '</ul>
											</div>
										</div>';
									}
								...
								foreach ($datos_publicador as $territorioscampa) {
									if(!empty($territorioscampa['ID_publicador_campaing']) AND $territorioscampa['ID_publicador_campaing'] == $territorioscampa['id']){
										...
										<div class="col-md-4 animation">
											...
											;
											if($territorioscampa["imagen"]==null){
												echo ' <img src="'.CDN.'/images/sinimagen.jpg" class="card-img-top" alt="No existe imagen del territorio">';
											}else{
												echo '<img src="'.$territorioscampa["imagen"].'" class="card-img-top" alt="No existe imagen del territorio">';
											}
											  echo '
											  <div class="card-body">
											    ...
											    if($territorioscampa['devuelta_campaing']<= "20".date('y-m-d')){
											    	echo '<span class="alerta">Territorio cumplido</span>';
											    	$cumplidos=true;
											    	array_push($array_terri_cumplidos,["numero"=>$territorioscampa["numero_territorio"],"zona"=>$territorioscampa["zona"],"devuelta"=>$territorioscampa["devuelta_campaing"],"modo"=>"campaing"]);
											    }
											    echo '</div>
											  <ul class="list-group list-group-flush">
											    <li class="list-group-item">
											    ...
											    <li class="list-group-item"><button class="btn btn-secondary" type="button" onclick="ReqDatos.devolver_territorio_campaing('.$campaingdata['ID'].','.$territorioscampa['idterritorios'].','.$territorios['id'].')">Devolver territorio</button></li>
											  </ul>
											</div>
										</div>';
									}
			...
			<div class="row">
				<div class="col-md-12 text-right">
					<?php
						if($cumplidos){ 
							$data =json_encode($array_terri_cumplidos);
							//print_r($data);
					?>			
							...
							<span class="numeroterri">Hermano con territorios cumplidos:</span>
							<button type="button" class="btn btn-light notification" onclick="ReqDatos.notificar_publicador(obj)">Enviar email de notificacion</button>
					<?php } ?>
				</div>
			</div>
	</div>
